Feature: Query canceled features
  As a website manager, I want to get all canceled trades in our platform, 
  so that I can check trades' information.


  Scenario Outline: Manager successfully query all canceled trades (Normal Flow)
  ...
  Scenario Outline: Manager failed to query all completed trades. (Error Flow)
    ...
    Then an error message should be displayed for failing to query all canceled trades